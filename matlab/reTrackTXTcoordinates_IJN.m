...
%Jeff Rhoades changed 5/3/17, so many things (made callable function, batch
...
%can use to specify larger square for integration. (sqSize)
    ...
    while ~strcmp(success, 'Yes') %while comparing strings success and yes, loop should continue until success == yes
        day = uigetdir; %Pull up user interface for directory selection
        vids = dir(day); %list all files and folders in directory
        vids = vids(3:end); 
        vids = vids([vids.isdir]); % select only folders within the directory
        vids = arrayfun(@(x) [x.folder '\' x.name], vids, 'UniformOutput', false);% @(x)creates anonymous function that combines individual strings to form directory name. Function is applied to all elements in array
        
        for d = 1:length(vids)
            pn = vids{d};
            cd(pn); %open folder/directory
            animals = dir('*\*.txt'); % list all txt files in chosen folder

            if isempty(animals)
                %select folder with original tifs, w/ subfolder with old track files
                pn = uigetdir;
                cd(pn);
                animals = dir('*\*.txt'); % opens all text file WITHIN subfolder (usually sub folder where background has been subtracted)
            end
            ...
            for k = 1:length(animals)
                ...
                D = dir('*.tif');% create list of image files (usually uncorrected) in folder
                fn = D(1).name; % assign file name of first image in sequence to fn
                movieFile = [pn fn]; %concatenate the strings to form full address+name of first file in sequence. Will disrupt analysis if pn does not contain other folders
                ...
                [xMesh, yMesh] = meshgrid(1:imReadSize*2+1, 1:imReadSize*2+1); % creates two 2D grids(look up the function)
                ...
                for i = 1:length(D)
                    ...
                    IndexLast=find(dat(:,1)==i,1,'last'); %for each image file, find representative row number in text file (keep the last one)
