...
%     thresh = 2.5;
    ...
    data = getVelocities(data, frameRate, pixelSize);%NOTE: Just pixel based, not scaled
%     data = cleanFluors(data, thresh); DONE LATER IN DISPLAY SCRIPT
    ...
    defFilename = strsplit(defFilename{1}, '_');
...
function [data, prompt, refeedsInds] = getRefeeds(data)
    ...
    refeedsInds = inputdlg(prompt);
    i = 1;
    for s = 1:length(strains)%each strain
        for w = 1:length(data.(strains{s}))%each worm
            thisRefeed = str2num(refeedsInds{i});
            if isempty(thisRefeed)
                thisRefeed = data.(strains{s})(w).Frame(1);
            end
            ...
            for e = 1:(length(thisRefeed) - 1)
                dife = abs(data.(strains{s})(w).Frame - thisRefeed(e));
                difeN = abs(data.(strains{s})(w).Frame - thisRefeed(e+1));
                startInd = find(dife == min(dife), 1);
                endInd = find(difeN == min(difeN), 1);
                data.(strains{s})(w).refed(startInd : endInd) = mod(e,2);
            end
            i = i + 1;
        end
    end
    
% OLD VERSION W/O RE-ENCOUNTERS/LAWN LEAVINGS
%     for s = 1:length(strains)%each strain
%         for w = 1:length(data.(strains{s}))%each worm
%             prompt(i) = {sprintf('Encounter Frame for %s', data.(strains{s})(w).name)};
%             i = i + 1;
%         end
%     end
%     
%     refeedsInds = inputdlg(prompt);
%     refeedsInds = cellfun(@(x) str2double(x), refeedsInds);
%     i = 1;
%     for s = 1:length(strains)%each strain
%         for w = 1:length(data.(strains{s}))%each worm
%             data.(strains{s})(w).refed = data.(strains{s})(w).Frame >= refeedsInds(i);
%             i = i + 1;
%         end
%     end
