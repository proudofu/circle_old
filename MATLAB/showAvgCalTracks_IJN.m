function [strainMats, avgs, stdErr] = showAvgCalTracks(calTracks, medWindow, varargin)
    ...
    for s = 1:length(strains)%get pretty data
        ...
        if ~indie
            ...
            %HeatMap(cleanTracks.fluors, 'ColumnLabels', cleanTracks.refed, 'Standardize', 2);
            fig = gcf;
            figure
            %imagesc([cleanTracks.refed*max(max(cleanTracks.fluors)); cleanTracks.fluors], clim);
            ...
            %plot([preSpan preSpan], [0 length(calTracks.(strains{s}))+ 2], 'Color', 'k', 'LineWidth', 0.7);
        else
